...
internal sealed class TestPki : IDisposable
{
    ...
    /// <param name="intermediateCrlDistributionPointUrl">A CRL distribution point for the intermediate, whose CRL the
    /// root issues (<see cref="BuildRootCrl"/>).</param>
    public TestPki(bool useRsa = false, string intermediateCrlDistributionPointUrl = null)
    {
        _useRsa = useRsa;
        _rootKey = CreateKey();
        ...
        // The names carry a unique suffix so that no run can be served a CRL another run left in the platform's cache
        ...
        _intermediateWithKey = IssueIntermediate($"CN=Test Intermediate {suffix}", _intermediateKey, crlDistributionPointUrl: intermediateCrlDistributionPointUrl);
        Intermediate = WithoutKey(_intermediateWithKey);

        using var lookalikeWithKey = IssueIntermediate($"CN=Test Intermediate {suffix}", _lookalikeKey, subjectKeyIdentifierOf: _intermediateWithKey);
        IntermediateLookalike = WithoutKey(lookalikeWithKey);

        Leaf = IssueLeaf();
        ExpiredLeaf = IssueLeaf(notBefore: _now.AddDays(-2), notAfter: _now.AddDays(-1));

        EmptyCrl = BuildCrl(_now.AddDays(7), revokeLeaf: false);
        CrlRevokingLeaf = BuildCrl(_now.AddDays(7), revokeLeaf: true);
    }
    ...
    /// </summary>
    public byte[] BuildCrl(DateTimeOffset nextUpdate, bool revokeLeaf, HashAlgorithmName? hashAlgorithm = null)
    {
        var builder = new CertificateRevocationListBuilder();

        if (revokeLeaf)
        {
            builder.AddEntry(Leaf);
        }

        return Build(builder, _intermediateWithKey, nextUpdate, hashAlgorithm ?? HashAlgorithmName.SHA256);
    }

    /// <summary>
    /// Builds the root's CRL, which covers the intermediate.
    /// </summary>
    public byte[] BuildRootCrl(DateTimeOffset nextUpdate, bool revokeIntermediate)
    {
        var builder = new CertificateRevocationListBuilder();

        if (revokeIntermediate)
        {
            builder.AddEntry(Intermediate);
        }

        return Build(builder, _rootWithKey, nextUpdate, HashAlgorithmName.SHA256);
    }

    private byte[] Build(CertificateRevocationListBuilder builder, X509Certificate2 issuer, DateTimeOffset nextUpdate, HashAlgorithmName hashAlgorithm)
    {
    ...
    private X509Certificate2 IssueIntermediate(string subject, AsymmetricAlgorithm key, X509Certificate2 subjectKeyIdentifierOf = null, string crlDistributionPointUrl = null)
    {
        ...
        if (crlDistributionPointUrl is not null)
        {
            request.CertificateExtensions.Add(CertificateRevocationListBuilder.BuildCrlDistributionPointExtension([crlDistributionPointUrl]));
        }
